<x-app-layout>
    <div class="container mx-auto">
        <div class="sm:grid mx-2 grid-cols-12 ">
            <form action="{{route('video.update',$video->id )}}" method="POST" class="mt-2 col-span-8 col-start-3 block" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="mb-6">
                    <label for="title" class="block mb-2 text-sm font-medium text-gray-900">{{__("العنوان")}}</label>
                    <input type="text" name="title" id="title" value="{{$video->title}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                    @error('title')
                ...
                <div class="mb-6">
                    ...
                    <textarea type="text" name="desc" id="desc" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">{{$video->desc}}</textarea>
                    @error('desc')
                        <p class="text-xl text-red-500">{{$message}}</p>
                    @enderror
                </div>
                <div class="mb-6">
                    ...
                    <div class="flex mt-3 items-center justify-center w-full">
                        <label for="dropzone-image" class="flex flex-col items-center justify-center w-full h-52 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 ">
                            ...
                            <input onchange="readCoverImage(this)" id="dropzone-image" name="image_path" type="file" class="hidden" />
                        </label>
                    </div>
                     <div class="my-8 w-48  h-32 border-2">
                        <img class="h-full w-full" src="{{$video->image_path}}" alt="" id="cover-image-thumb">
                    </div>
                    @error('image_path')
                        <p class="text-xl text-red-500">{{$message}}</p>
                    @enderror
